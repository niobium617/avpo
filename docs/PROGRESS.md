...
**M11 工作台多任务并行 —— 11.1~11.5 全部完成**（357 测试全绿 + 2 live 跳过）

| 任务 | 产出 | 状态 |
|---|---|---|
| 11.1 tasks.py | TaskContainer 增 pid 字段（UI 任务槽 key 同源）+ 线程名 avpo-{pid}-{node} + 模块 docstring 记 M11 并发语义 | ✅ |
| 11.2 每项目任务槽 | `session_state["task"]` → `tasks` dict[pid]；`_project_busy(pid)` 助手；`_start_node/_start_chain` 按项目守卫（同项目互斥警告）；全部 busy 站点（策划/分镜/流水线）改按项目 | ✅ |
| 11.3 进度区与结果 | `_render_one_task` 抽出（返回快照 state 供消费判断，无裸属性读）；fragment 遍历全部任务、终态一轮消费只 rerun 一次；`task_results` dict[pid]：项目页只显当前项目结果，项目管理页带 pid 标签汇总 | ✅ |
| 11.4 测试 | 既有断言改写（`_wait_task`/`_no_task` 带 pid，tasks/task_results 引用 9 处）+ 新增 4 条（双项目并行双慢 mock/按项目 busy 编辑不锁/结果按项目隔离/管理页汇总） | ✅ **357 全绿** |
| 11.5 文档 | README M11 章 + 工作台表/后台任务模型/架构图/目录/里程碑表 + 已知限制更新；本文件 + 路线图 | ✅ |

要点（M11）：
- **同项目互斥、跨项目并行**：任务槽按项目 —— 同一 project.json 的两个并发任务
  会互相覆盖（竞写），同项目仍单任务（按钮禁用 + 启动守卫双保险）；不同项目
  目录天然隔离，save 的进程内锁（M5 `_SAVE_LOCK`）序列化原子写与 git 提交，
  跨项目并行安全（11.1 无需新锁，沿用 M5 基建）。
- **busy 按项目**：项目 A 跑长任务时 A 的按钮/编辑禁用（防 worker 副本覆盖），
  B 的策划/分镜/流水线照常 —— 创作者不再干等生图。
- **结果不串味**：`task_results[pid]` 持久结果按项目取用；项目管理页无当前项目
  概念，带 pid 标签汇总（任务在管理页完成也能看到反馈）。
- **无 schema/依赖变更**：schema 0.6 不动、versions.md 不动；CLI 天然单任务不受影响。
- **测试基线修正**：M10 关闭时记录「354 全绿」，本次经 HEAD 收集核对实为 353
  （M11 起以 pytest 收集数为准：353 + 4 = 357）。
- **已知限制**：多浏览器会话对同一项目并发启动任务不受 UI 守卫保护（各自任务
  互不知晓，后存者覆盖 —— M5 起即有的多会话边界，README 已知限制在案）。

### 下一步

- 候选池：SQLite 迁移；PR/DaVinci XML（优先级低于剪映）；题材模板积累（templates/）。

---

## M10 本地字幕（2026-08-30）—— 10.1~10.10 全部完成
...
- **0.3s 固定**：`_TRANSITION_MS`（export/jianying.py）单点可调；闪白 = 非叠加
  （切点处播放）、震动 = 叠加（剪映原生元数据，编辑器自处理叠化窗口）——剪映
  9.7.1 打开验证通过（opened_log 2026-08-29：全闪白 5 切点 + 混合对照震动叠化
  正常/显式 none 硬切，可编辑保存）。
