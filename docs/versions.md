...
| faster-whisper | 1.2.1 | 2026-08-30 | **M10 钉版**（ctranslate2 4.8.1 + av 18.1.0，py3.13 wheel 实测）。本地转写：`WhisperModel(device="cpu", compute_type="int8")`；模型经 `download_model(cache_dir=<数据目录>/whisper_models)` 下载（HF_HOME 重定向禁写 C 盘，`HF_ENDPOINT` 可配镜像）。模型尺寸 tiny/base/small/medium/large-v3 可换（默认 small） |
